% !TeX root = ../../main.tex
\subsection{HT-29 serial harvests provide descriptive trajectory scores}
...
We summed the three sequencing columns at each late day because they assay the
same harvested population.  The seven harvests are likewise repeated
...
descriptive trajectory scores.  Both methods used the same eight-column matrix
and numeric design.  We also report the deposited all-time-point Chronos score
and day-25 MAGeCK and BAGEL2 scores
...
Official continuous-time MAGeCK has the highest PR AUC, Chronos has the most
negative NNMD, and BAGEL2 has the fewest unexpressed genes in the 10\% tail.
These metrics favor different score properties, but all describe a single,
unreplicated biological lineage.

Taking the median MAGeCK effect across all seven days instead of the day-25
column lowers AUROC from 0.9792 to 0.9742 and Recall$_{90}$ from 0.9120 to
0.8602.  Early harvests carry little depletion, so an across-day median
attenuates the endpoint signal. This choice of summary changes performance
more than the differences among trajectory scores, which limits the use of
this dataset as inferential evidence for a count model.

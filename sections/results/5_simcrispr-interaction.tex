...
never used for evaluation.  This is a within-BARCS ablation of denominator
choice and moderation.  MAGeCK-MLE is included as a reference with one guide
per gene; that input lacks within-gene replication for its native gene model
and cannot support a method-ranking conclusion.
...
the interquartile range, whiskers extend to 1.5 times it, and the number of
observations beyond the whiskers is stated in the panel rather than plotted.
(B) F1 across nominal FDR thresholds; every evaluated threshold is marked.  Control denominators retain
substantially more interaction power.  The three denominators are one estimator
run three times and therefore share a hue, separated by lightness, line type,
and point shape; MAGeCK-MLE carries its own colour, consistent with the other
figures, and appears in (B) only.}
\label{fig:simcrispr}
\end{figure}
...
In these simulations, both control classes supported targeted-interaction
recovery, while safe-harbor normalization absorbed more of the shared cutting
response.  A comparison of gene-level interaction methods would require
several independently simulated guides per gene and a common gene-level

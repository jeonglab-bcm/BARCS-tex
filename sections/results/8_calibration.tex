...
To test whether calibration changes after aggregation, we simulated all-null
...
particularly with five guides per gene.  The gene combiner's independence
assumption therefore contributes to miscalibration.  A near-nominal
guide-level diagnostic alone cannot establish calibration of the gene
probabilities.
...
genes after the same aggregation.  The Mod. column denotes guide-variance moderation.  Rejection uses $p<0.05$, with a target rate of 0.05.}
...
BARCS produced different diagnostics for the deposited normalized PSN1 and
DeWeirdt tables.  PSN1 produced no gene calls and guide-level $p<0.05$ rates
of 0.0481 before scaling and 0.0465 after scaling.  In DeWeirdt, BARCS
recovered one of 28 curated interactions, and 37 of its 38 calls arose in one
comparison. The complete audit is reported in Supplementary
Section~\ref{sec:supp-fdr-audit}.  Interpreting a discovery threshold requires
checking the denominator, guide dependence, and calibration on independent
controls.

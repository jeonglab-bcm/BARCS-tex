...
We downloaded the 64,747-guide count matrix for GSE70038 from GEO
...
directional Stouffer statistic; the gene effect was the median guide
coefficient.  For the overlap analysis, we selected the 200 most negative
finite-effect genes from each method for each coefficient, partitioned the
equal-sized lists into shared and method-specific sets, and queried GO
Biological Process 2023 and Reactome 2022 through Enrichr
\citep{chen2013enrichr}.  We also used one-sided Fisher tests to measure
enrichment of each exclusive set for the Hart reference-essential genes
\citep{hart2017evaluation} against the genes testable by both methods.
